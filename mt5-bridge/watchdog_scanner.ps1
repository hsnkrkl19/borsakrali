...
# Durdurmak: mt5-bridge klasorune STOP_SCANNER dosyasi koy (bot emir acmaz)
...
if ($zaten) {
    Add-Content watchdog_scanner.log "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') zaten calisiyor (PID $($zaten.ProcessId)) - bekci cikti"
    exit
}
...
while ($true) {
    if (-not (Get-Process terminal64 -ErrorAction SilentlyContinue)) {
        if (Test-Path $MT5_EXE) {
            Add-Content watchdog_scanner.log "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') MT5 terminali kapali - baslatiliyor"
            Start-Process $MT5_EXE
            Start-Sleep -Seconds 30   # terminal login olsun
        }
    }
    Add-Content watchdog_scanner.log "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') bot baslatiliyor"
    & python -X utf8 borsakrali_mt5_scanner.py
    Add-Content watchdog_scanner.log "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') bot cikti (exit=$LASTEXITCODE) - 60 sn sonra yeniden"
