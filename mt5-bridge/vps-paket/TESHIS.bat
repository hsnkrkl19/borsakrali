...
>> "%RAPOR%" echo --- DOSYALAR ---
for %%F in (borsakrali_mt5_all.py borsakrali_mt5.py borsakrali_mt5_scanner.py trade_guard.py config_all.json config.json config_scanner.json) do (
...
wmic process where "name='python.exe'" get commandline /format:list >> "%RAPOR%" 2>&1
>> "%RAPOR%" echo.
