...
import { Newspaper, RefreshCw, AlertCircle, ExternalLink, Bitcoin, Globe, Filter, Clock } from 'lucide-react'
...
function NewsCard({ item }) {
  ...
  return (
    <a
      ...
      className={`block rounded-xl border bg-dark-900/50 hover:bg-dark-900/80 ${accentColor} p-3 transition-all group`}
    >
    ...
    <div className="space-y-4 max-w-4xl mx-auto">
      ...
      {filteredNews.length > 0 && (
        <div className="space-y-2">
          {filteredNews.map(n => (
            <NewsCard key={n.id + n.title} item={n} />
          ))}
        </div>
      )}
