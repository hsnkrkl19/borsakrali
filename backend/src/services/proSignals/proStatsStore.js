/**
 * proStatsStore — YENİ ROBOT sinyal KAYDI + enstrüman bazlı başarı istatistiği.
 *
 * forexStatsStore'un BİREBİR klonu; tek fark AYRI kalıcılık (forex ile asla
 * çakışmaz), proSignals enstrüman kümesi ve "YENİ ROBOT" başlığı.
 *
 * - Her açılan pozisyon (sinyal) ve her kapanış geçmişe kaydedilir (history).
 * - Kapanışlarda enstrüman+yön bazlı kazanan/kaybeden sayılır → başarı oranı.
 *   Başarı = kârla kapanan (TP1 / iz-süren stop / +pnl ile süre dolması).
 * - Günde 2 kez (cron) `buildStatsMessage` ile kanala paylaşılır.
 *
 * Kalıcılık: Supabase 'bot-state'/'pro-signals/stats.json' + disk (tracker ile aynı).
 */
const fs = require('fs');
const path = require('path');
const { listInstruments } = require('../proSignals/proInstruments');
...
const SUPA_KEY = 'pro-signals/stats.json';
const DISK_FILE = path.join(__dirname, '..', '..', 'data', 'pro-signals-stats.json');
...
async function load() {
  ...
  // Tek-seferlik SIFIRLAMA: PRO_STATS_RESET değeri (etiket) değişince istatistiği
  ...
  const tag = process.env.PRO_STATS_RESET;
...
// Telegram istatistik mesajı (enstrüman bazlı long/short başarı oranı)
async function buildStatsMessage() {
  ...
  const lines = [`🤖 <b>YENİ ROBOT SİNYAL İSTATİSTİĞİ</b> — ${today}`];
