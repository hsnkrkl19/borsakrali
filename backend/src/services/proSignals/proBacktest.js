/**
 * proBacktest — YENİ ROBOT (Derin Konfluans) sinyalleri için GEÇMİŞ BAŞARI
 * (backtest) + KALİBRASYON. forexBacktest'in birebir KLONU; tek fark sinyal
 * üretimi 5-strateji yerine proConfluence DERİN KONFLUANS motoruyla yapılır.
 *
 ...
 *
 * Ağır iştir → günde 1 kez (cron) çalışır, diske yazılır; canlı proEngine sonucu
 ...
 *
 * NO-LOOKAHEAD (kritik): bar i için sinyal yalnız candles.slice(0, i+1)'i görür;
 * ileri değerleme yalnız candles.slice(i+1, i+1+horizon)'u görür. analyzeTF async
 * olduğundan bar başına `await` ile (Promise.all DEĞİL — bellek için) koşulur.
 */
...
const { computeConfidence, bandFor } = require('../forex/forexAggregator');
const { INSTRUMENTS } = require('./proInstruments');
const sidecar = require('../backtest/sidecarClient');
const logger = require('../../utils/logger');

const STORE_FILE = path.join(__dirname, '..', '..', 'data', 'pro-signals-backtest.json');
const TFS = ['15m', '1h', '4h', '1d'];
const HORIZON = { '15m': 16, '1h': 12, '4h': 10, '1d': 8 }; // ileri bar
...
// Ağırlık vektörü: self-test (varsa) tuned, yoksa BASE_WEIGHTS. proSelfTest henüz
// olmayabilir / döngü olabilir → lazy require + try/catch.
function getWeights() {
  try {
    const st = require('./proSelfTest');
    if (st && typeof st.getTunedWeights === 'function') {
      const w = st.getTunedWeights();
      if (w) return w;
    }
  } catch (_) { /* yok / döngü → taban */ }
  return proConfluence.BASE_WEIGHTS;
}
...
// Bir (enstrüman, TF) için geçmiş sinyalleri ÜRET (derin konfluans motoruyla) —
// sidecar'a gönderilecek ham seri. Heavy: bar başına TÜM teknikler koşar.
// analyzeTF async → bar başına AWAIT (Promise.all DEĞİL: bellek/lookahead güvenliği).
async function buildSignals(inst, tf, candles, opts = {}) {
  ...
  for (let i = start; i < end; i++) {
    ...
    const a = await proConfluence.analyzeTF(hist, { id: inst.id, tf: `${tf}#bt${i}`, class: inst.class, assetType, weights });
    if (a.status !== 'signal' || a.direction === 'neutral') continue;
    // Kapısız ham seviyeler → tarihsel örneklem korunur (canlı motorun rr<1.6 kapısı YOK)
    const lv = proLevels.buildLevelsRaw(a.direction, hist[i].close, a.atr, tf, inst.precision, hist);
    if (!lv) continue;
    // Canlı motorla AYNI: per-TF backtest'te cross-TF confluence yok → confluence:0;
    // ham güvene regimeMultiplier uygula (volatilite kapısı).
    let conf = computeConfidence({
      consensus: a.components.consensus,
      avgScore: a.components.avgScore,
      trendStrength: a.components.trendStrength,
      momentum: a.components.momentum,
      rr1: lv.rr1,
      confluence: 0,
    });
    conf = Math.round(conf * (a.regimeMultiplier ?? 1));
    if (conf < MIN_CONFIDENCE) continue;
    signals.push({ index: i, direction: a.direction, entry: hist[i].close, stop: lv.stop, target: lv.target1, band: bandFor(conf), atr: a.atr });
  }
...
async function runAll(opts = {}) {
  ...
  for (const { inst, tf } of subset) {
    ...
    } catch (e) { logger.warn(`proBacktest ${inst.id}:${tf} hata: ${e.message}`); }
  }
...
// Bant seçimi canlı motorla AYNI bandFor(confidence) → kalibrasyon tutarlı.
