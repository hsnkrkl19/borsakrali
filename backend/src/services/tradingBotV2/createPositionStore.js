/**
 * createPositionStore — paylaşılan sanal-portföy deposu fabrikası
 *
 * Hem BIST botu (tradingBotV2) hem Kripto botu (cryptoBotV2) AYNI disk-üstü
 * JSON deposunu kullanır; tek fark veri alt-dizini ve başlangıç sermayesidir.
 * Böylece iki bot da aynı battle-tested CRUD/portföy/sinyal-log mantığını
 * paylaşır, kod tekrarı olmaz.
 *
 * Veri dizini env ile değiştirilebilir: `BOT_DATA_DIR` set edilirse veriler
 * oraya yazılır (Render kalıcı disk eklenince tek satır env değişikliğiyle
 * geçmiş kalıcı olur — kod değişmez).
 *
 * Dosyalar: <DATA_ROOT>/<subdir>/
...
function createPositionStore({ subdir, initialCapital = 100000, currency = 'TRY' } = {}) {
  if (!subdir) throw new Error('createPositionStore: subdir gerekli');
  ...
  function readJSON(file, fallback) {
    ...
    try {
      ...
      console.error(`[PositionStore:${subdir}] Bozuk dosya: ${file}`, e.message);
      return fallback;
    }
  }
  ...
  function getPortfolio() {
    ...
    if (!p.currency) p.currency = currency; // eski dosyalara currency ekle
    return p;
  }
