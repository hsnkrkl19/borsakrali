/**
 * Crossover Store — EMA34/TEMA34 kırılım bildirimcisinin durum deposu.
 *
 * Tek dosya (data/crossover-alerts/runs.json) tutar:
 ...
 *
 * botPersistence ile 'crossover-alerts/runs.json' Supabase Storage'a yedeklenir;
...
function read() {
  try {
    ...
    console.error('[Crossover] state okuma hatası:', e.message);
    return emptyState();
  }
}
...
function write(state) {
  ...
  try {
    ...
    console.error('[Crossover] state yazma hatası:', e.message);
  }
